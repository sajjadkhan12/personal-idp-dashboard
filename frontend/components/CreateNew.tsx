...
const TemplateCard: React.FC<{ template: SoftwareTemplate, onSelect: () => void }> = ({ template, onSelect }) => (
  <button onClick={onSelect} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 p-6 text-left flex flex-col items-start h-full cursor-pointer">
    <div className="flex items-center mb-4">
      {template.icon}
      <h3 className="ml-4 font-bold text-slate-800 text-lg">{template.name}</h3>
    </div>
    <p className="text-sm text-slate-600 flex-grow mb-4">{template.description}</p>
    <div className="flex flex-wrap gap-2">
      {template.tags.map(tag => (
        <span key={tag} className="inline-block px-2 py-1 text-xs leading-none bg-slate-100 text-slate-600 rounded-full font-semibold uppercase tracking-wide">{tag}</span>
      ))}
    </div>
  </button>
);
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="relative flex-grow">
            ...
            <input 
              ...
              className="bg-white border border-slate-300 rounded-md py-2 pl-10 pr-4 w-full sm:w-80 focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
              value={searchQuery}
